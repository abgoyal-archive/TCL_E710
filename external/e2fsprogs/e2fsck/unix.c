			     ...
			     ext2_filsys *ret_fs)
{
	...
	*ret_fs = NULL;
    ...
    {   
        int sb_offset[] = {0,8193, 24577, 40961, 57345, 73729, 204801, 221185, 401409};
        int sb_offset4K[] = {0,32768};
        int superblock;
        for (superblock = 0; superblock < 5; superblock++) 
        {
            ctx->superblock = sb_offset[superblock];
            printf( _("superblock offset = %d\n"),ctx->superblock);
		    retval = ext2fs_open2(ctx->filesystem_name, ctx->io_options, 
			      flags, ctx->superblock , 1024, io_ptr, ret_fs);
            //printf( _("ext2fs 1K_open2 retval = %d\n"), retval ); 
            
            if (retval != EXT2_ET_BAD_MAGIC && retval != EXT2_ET_UNEXPECTED_BLOCK_SIZE)
                break;
            ...
            if(superblock <=1)//4k check 0 and 32768
            {
                ctx->superblock = sb_offset4K[superblock];
                printf( _("superblock offset = %d\n"),ctx->superblock);
    		    retval = ext2fs_open2(ctx->filesystem_name, ctx->io_options, 
    				      flags, ctx->superblock , 4096, io_ptr, ret_fs);
                //printf( _("ext2fs_4K open2 retval = %d\n"), retval );       
                if (retval != EXT2_ET_BAD_MAGIC && retval != EXT2_ET_UNEXPECTED_BLOCK_SIZE)
                    break;
    			if (*ret_fs) {
    				ext2fs_free(*ret_fs);
