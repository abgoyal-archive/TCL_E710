				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		...
		rk610_codec_write(codec, ACCELCODEC_R1F, 0xEF);
		break;
...
/*-------------------------lgw@tcl-------------------------*/
extern void turn_on_speaker();
extern void turn_off_speaker();

extern int headset_in(void);
static int spk_ref = 0;
/*----------------------END--------------------------------*/
static int rk610_codec_pcm_startup(struct snd_pcm_substream *substream,
...
static int rk610_codec_pcm_shutdown(struct snd_pcm_substream *substream,
                              struct snd_soc_dai *dai)
{
        /*--------------------lgw@tcl-----------------------*/
        spk_ref--;
...
static struct snd_soc_dai_ops rk610_codec_ops = {
	.startup = rk610_codec_pcm_startup,
	.shutdown = rk610_codec_pcm_shutdown,
...
static int rk610_codec_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	rk29_mux_api_set(GPIO0B1_I2S8CHSCLK_NAME, GPIO0B_GPIO0B1);
	gpio_request(I2S_SCLK, "i2s_sclk pin");
	gpio_direction_output(I2S_SCLK, GPIO_LOW);
//	rk610_codec_reg_read();
	return 0;
}
...
static int rk610_codec_resume(struct snd_soc_codec *codec)
{
	...
	rk29_mux_api_set(GPIO0B1_I2S8CHSCLK_NAME, GPIO0B_I2S_8CH_SCLK);
//	turn_on_speaker();
	DBG("Enter::%s----%d\n",__FUNCTION__,__LINE__);
