...
// cmy:
	...
	{ USB_DEVICE(ALCATEL_VENDOR_ID, 0x0192) },	//for M800
	{ USB_DEVICE(ALCATEL_VENDOR_ID, 0x0195) },	//for Y800
	{ USB_DEVICE(ALCATEL_VENDOR_ID, 0xf017) },	//for W800
	{ USB_DEVICE(ALCATEL_VENDOR_ID, 0x0197) },	//01-14
...
//miaozh add
static struct proc_dir_entry *usbpid = NULL;
static char product_id[8]={0};
static int usbpid__read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len;
...
static int __init option_init(void)
{
	...
	printk(KERN_INFO KBUILD_MODNAME ": " DRIVER_VERSION ":"
	       DRIVER_DESC "\n");
//miaozh add
		usbpid = create_proc_entry("usbpid", 0664, NULL);
		if( usbpid )
			{
				usbpid->read_proc = usbpid__read_proc;
				printk("create_proc_entry ok\n");
			}
			else
				printk("create_proc_entry fail\n");
//end by miaozh		
	return 0;
...
//miaozh add
			printk("serial->dev->descriptor.idVendor=0x%04x, idProduct=0x%04x\n", serial->dev->descriptor.idVendor, serial->dev->descriptor.idProduct);
	if (serial->dev->descriptor.idVendor == ALCATEL_VENDOR_ID)
		{
			sprintf(product_id, "%04x\n", serial->dev->descriptor.idProduct);
			printk("product_id = %s\n", product_id);
		}
	//end by miaozh
