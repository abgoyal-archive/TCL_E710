...
//miaozh add for wifi sleep power consumption
...
//add by wfy ------for first time sd card detect---------------
void first_time_sd_detect(struct rk29_sdmmc* host)
{
	...
	printk("%s:--present:%s,old:%s",mmc_hostname(host->mmc),present?"true":"false",present_old?"true":"false");
...
extern int rk29sdk_wifi_reset(int on);
...
//miaozh add for wifi cause the null pointer crash	
	rk29sdk_wifi_reset(0);
	rk29sdk_wifi_reset(1);
#if defined(CONFIG_RK29_SDIO_IRQ_FROM_GPIO)	
...
//add by wfy for suspend current inc to 40mA when sd card insert--------
	if((!strncmp(mmc_hostname(host->mmc),"mmc0",4) && host->set_iomux))
		host->set_iomux(0,0x1234);
//----------------------------------------------------------------------
#if !defined(CONFIG_SDMMC0_RK29_SDCARD_DET_FROM_GPIO)
	rk29_sdmmc_enable_irq(host,false);		//bruins test
    rk29_mux_api_set(host->det_pin.iomux.name, host->det_pin.iomux.fgpio);
	gpio_request(host->det_pin.io, "sd_detect");
	gpio_direction_output(host->det_pin.io, GPIO_HIGH);		//bruins test
	gpio_direction_input(host->det_pin.io);
...
//----------------------------------------------------------------------
	...
	rk29_sdmmc_enable_irq(host, true);//bruins test
#endif
}
