...
int rk29_bl_update_status(struct backlight_device *bl)
{
	u32 divh,div_total;
...
int rk29_bl_update_status_zero(struct backlight_device *bl)
{
	u32 divh,div_total;
	struct rk29_bl_info *rk29_bl_info = bl_get_data(bl);
	u32 id = rk29_bl_info->pwm_id;
...
static void rk29_bl_suspend(struct early_suspend *h)
{
	...
	turn_off_GPS();		//wanghui,2013-3-11
...
/*---------lgw@tcl-------------------*/
extern int battery_power_low(void);
static void rk29_bl_resume(struct early_suspend *h)
{
	struct rk29_bl_info *rk29_bl_info = bl_get_data(rk29_bl);
	DBG("%s : %s\n", __FILE__, __FUNCTION__);
	
	turn_on_GPS();		//wanghui,2013-3-11
	/*--------lgw@tcl---------*/
	if (battery_power_low())
		return;
