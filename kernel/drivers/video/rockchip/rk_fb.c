...
#endif
struct platform_device *g_fb_pdev;
...
/*--------------------lgw@tcl-----------------------------------*/
extern int fb_prepare_logo_for_rockchip(struct fb_info *info, const struct linux_logo *logo, int rotate);
void logo_display(const struct linux_logo *logo)
{
	struct rk_fb_inf *inf = platform_get_drvdata(g_fb_pdev);
//	pr_info("%s,%d\n",__FUNCTION__,__LINE__);       
//      fb0_set_par(inf->fb0);
	if (fb_prepare_logo_for_rockchip(inf->fb[0], logo, FB_ROTATE_UR)) {
		fb_set_cmap(&inf->fb[0]->cmap, inf->fb[0]);

//		pr_info("%s,%d\n",__FUNCTION__,__LINE__);
                fb_show_logo(inf->fb[0], FB_ROTATE_UR);
