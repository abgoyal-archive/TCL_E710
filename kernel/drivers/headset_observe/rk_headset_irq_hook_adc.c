...
int get_adc2_value(void)
{
	return adc_sync_read(headset_info->client);
}
...
static void headsetobserve_work(struct work_struct *work)
{
	...
	//	if(level < 0 && level == 0)
	//	{
			printk("%s:get pin level again,pin=%d,level=%d,i=%d\n",__FUNCTION__,pdata->Headset_gpio,level,i);
			msleep(1);
	//		continue;
	//	}
	//	else
...
*/
	...
	case HEADSET_IN_LOW:
		if(level == 0)
		{//in--High level
			/*DBG*/printk("---HEADSET_IN_LOW headset in LOW ---\n");
			...
			/*-----------wh----------------*/	
//			gpio_set_value(MIC_SW, GPIO_LOW);	//switch to HP_MIC
			headset_info->cur_headset_status = BIT_HEADSET_NO_MIC;
			headset_change_irqtype(HEADSET,IRQF_TRIGGER_RISING);//
		...
		{//out--High level
			...
			/*------------wh@tcl--------------*/
//			gpio_set_value(MIC_SW, GPIO_HIGH);
			del_timer(&headset_info->hook_timer);				
			headset_info->cur_headset_status = ~(BIT_HEADSET|BIT_HEADSET_NO_MIC);
			headset_change_irqtype(HEADSET,IRQF_TRIGGER_FALLING);//
			rk28_send_wakeup_key();
...
static void hook_adc_callback(struct adc_client *client, void *client_param, int result)
{
	...
	/*DBG*/pr_info("---hook_adc_callback---, result = %d, flag = %d\n", result, headset->adc_callback_status);
	...
	switch(headset->adc_callback_status)
	{
		...
		case HEADSET_TIMER:
			...
			if(level >= 0 && level < HOOK_LEVEL_LOW)
			{
				...
				/*-----------wh-------------*/
				gpio_set_value(MIC_SW, GPIO_HIGH);	//switch to MAIN_MIC
			}	
...
//	headset->input_dev->evbit[0] = BIT_MASK(EV_KEY);
	...
	if (pdata->Headset_gpio) {
		...
		if (ret) 
			goto failed_free_dev;
#else
	ret = gpio_request(pdata->Headset_gpio, "headset_gpio");
	if (ret < 0) {
	...
	gpio_direction_input(pdata->Headset_gpio);
		...
		disable_irq(headset->irq[HEADSET]);
#endif
	}
	else
		goto failed_free_dev;
...
/*--------------------lgw@tcl---------------------------*/
struct rk_headset_pdata rk_headset_device_platdata = {
	.Headset_gpio = RK30_PIN4_PC4,
	.headset_in_type = HEADSET_IN_LOW,
	.Hook_adc_chn = 2,
};
struct platform_device rk_headset_device = {
	.name = "rk_headsetdet",
	.id = -1,
	.dev = {
		.platform_data = &rk_headset_device_platdata,
	},
};
/*-----------------------END---------------------------*/
...
{
	/*------------------lgw@tcl------------------------------*/
	platform_device_register(&rk_headset_device);
