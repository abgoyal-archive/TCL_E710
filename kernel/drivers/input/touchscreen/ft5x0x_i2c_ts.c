...
//by huanghuabin
static struct timer_list ft5x0x_ts_timer;
static unsigned int touch_status_bk = 0;
//#define FT5X0X_DEBUG
...
//	static int key_id=0x80;
	...
	for (i=0; i<CFG_MAX_POINT_NUM; i++)     //add by huanghuabin
		_st_finger_infos[i].u2_pressure = 0;   
	
    buf[0] = 2;//ADDR
	ret = fts_i2c_rxdata(buf, 1 + 6 * CFG_MAX_POINT_NUM);
	if (ret > 0) 
...
//	printk(KERN_INFO "touch_point_num=%d\n", touch_point_num);
    ...
    if(touch_point_num != 0)
	{
        do
        {
            ...
            id = buf[1 + 2 + i_count * 6] >> 4;
            if((id < 0) ||  (id > CFG_MAX_POINT_NUM))
                printk("[ERROR] Touch ID readed is illegal!!\n");

            touch_event = buf[1 + i_count * 6] >> 6; 
            x =((buf[1 + i_count * 6] & 0x0f) << 8) |buf[1 + i_count * 6 + 1];
            y =((buf[1 + i_count * 6 + 2]& 0x0f) << 8) | buf[1 + i_count * 6 + 3];
	...
	if(touch_point_num == 0 )   //cancel by huanghuabin
	{
	    if(!touch_clear_ok) 
		    fts_ts_release();
            PDEBUG("------->up by ctp!\n");
	}   
    touch_status_bk = touch_status;	
	return 0;
}

static void fts_work_func(struct work_struct *work)
{
	disable_irq_nosync(this_client->irq);   //add by huanghuabin
...
static void ft5x0x_ts_do_timer(unsigned long arg)
{
	struct FTS_TS_DATA_T *data = i2c_get_clientdata(this_client);
...
static irqreturn_t fts_ts_irq(int irq, void *dev_id)
{
	...
	if (!work_pending(&ft5x0x_ts->pen_event_work)) {
		//disable_irq_nosync(this_client->irq);   //cancel by huanghuabin
...
{
//	#include "E710_ID85_V11_20121218_app.i"
};
...
void ft5x0x_ts_set_standby(struct i2c_client *client, int enable)
{
   ...
   //     if(fts_register_write(op[0],&op[1]) == FTS_FALSE)
   //         printk("---->%s : send sleep cmd error!\n",__FUNCTION__);
...
//#include "../../power/bq24195.h"
extern int bqSetFASTCHRG(int ichg);
extern int bqSetIINDPM(int idpm);
...
//miaozh add	
	printk("--------lite_delay_suspend_lock 5 secs---------\n");
	wake_lock_timeout(&lite_delay_suspend_lock, 5 * HZ);
...
static ssize_t show_irq_depth(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	unsigned long flags;
	int depth;

	struct irq_desc *desc = irq_get_desc_buslock(this_client->irq, &flags);

	depth = desc->depth;
	
	irq_put_desc_busunlock(desc, flags);

	return sprintf(buf, "depth=%d\n", depth);
}
static DEVICE_ATTR(irq_depth, 0644, show_irq_depth, NULL);
//----end
...
//	input_dev->evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS);
	...
	/****** for multi-touch *******/
	for (i=0; i<CFG_MAX_POINT_NUM; i++)   
		_st_finger_infos[i].u2_pressure = 0;    //-1;modify by huanghuabin   

	input_set_abs_params(input_dev,
			ABS_MT_POSITION_X, 0, 800/* + SCREEN_BOUNDARY_ADJUST_VALUE*/, 0, 0);
	input_set_abs_params(input_dev,
			ABS_MT_POSITION_Y, 0, 1280/* + SCREEN_BOUNDARY_ADJUST_VALUE*/, 0, 0);
//	input_set_abs_params(input_dev,
...
//	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
    ...
    ft5x0x_ts_timer.function = ft5x0x_ts_do_timer;
    ft5x0x_ts_timer.data = 0;
    ft5x0x_ts_timer.expires = 0;

	enable_irq(_sui_irq_num);   

	//lgw added
	device_create_file(&client->dev, &dev_attr_irq_depth);
