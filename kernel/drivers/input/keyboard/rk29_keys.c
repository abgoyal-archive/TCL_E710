...
//=add by wfy for wake up time too long----------------------
#define LOCK_POWER_KEY_WAIT_SUSPEND_ON 1 
#if LOCK_POWER_KEY_WAIT_SUSPEND_ON 
...
static struct delayed_work power_on_check;
static int power_key_waking;

static void power_key_check_func(struct work_struct *work)
{
	if(get_suspend_state()==0){
		printk("%s:wake up finish,unlock power key\n",__func__);
		wake_unlock(&power_key_lock);
		power_key_waking=0;
		return;
	}

	printk("%s:mayby power key lost,report again\n",__func__);
	rk29_send_power_key(1);
	rk29_send_power_key(0);
	
	schedule_delayed_work(&power_on_check,HZ/5);	
}
#endif
//---------------------------------------------------------------------
...
//=add by wfy for wake up time too long----------------------
	...
	INIT_DELAYED_WORK(&power_on_check,power_key_check_func);
#endif
//-----------------------------------------------------------
