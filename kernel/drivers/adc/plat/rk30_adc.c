...
extern struct rk30_adc_battery_data *gBatteryData;
extern void check_bat_vol(struct rk30_adc_battery_data *bat);
extern void ntc_adc_to_temp();
static int rk30_adc_resume(struct platform_device *pdev)
{
	struct rk30_adc_device *dev = platform_get_drvdata(pdev);

	dev->adc->is_suspended = 0;
	ntc_adc_to_temp();	// when adc is resumed, read value    wh@tcl	

	check_bat_vol(gBatteryData);
