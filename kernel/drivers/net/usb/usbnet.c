				     ...
				     struct net_device *net)
{
	...
	if (((uint32_t)skb->data & 3)) {
//modified by wfy---------rk patch 0608------------------------
#if 0
		if (!dev->usb_tx_buf) {
			printk("%s: usb_tx_buf = NULL.\n", __FUNCTION__);
			goto drop;
		}
		if(skb->len > MAX_USB_TX_BUF_SIZE) {
			printk("%s: usb_tx_buf size not enough: length = %d.\n", __FUNCTION__, skb->len);
			goto drop;
		}
		memcpy(dev->usb_tx_buf, skb->data, skb->len);
        skb->data = dev->usb_tx_buf;
#else
	memcpy(skb->data-2, skb->data, skb->len);
	skb->data = skb->data-2;
#endif 
//--------------------------------------------------------------
	}
